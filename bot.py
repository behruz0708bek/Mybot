...
BOT_TOKEN = "8738889310:AAG0vDNrAO9T2Xo6zQWJWHF8EmyFDTO1rnU"
ADMIN_ID = 7343837323

REQUIRED_CHANNELS = ["https://t.me/tarjiima_kinolar_kanali"]
...
async def show_series(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton(name, callback_data=f"series_{sid}")]
        for sid, name in data
    ]

    await update.callback_query.message.reply_text(
        "Choose series:", reply_markup=InlineKeyboardMarkup(keyboard)
    )


async def show_episodes(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query.data
    series_id = int(query.split("_")[1])

    cursor.execute("SELECT id, episode_name FROM episodes WHERE series_id=?", (series_id,))
    eps = cursor.fetchall()

    keyboard = [
        [InlineKeyboardButton(name, callback_data=f"ep_{eid}")]
        for eid, name in eps
    ]

    await update.callback_query.message.reply_text(
        "Episodes:", reply_markup=InlineKeyboardMarkup(keyboard)
    )


async def send_episode(update: Update, context: ContextTypes.DEFAULT_TYPE):
    eid = int(update.callback_query.data.split("_")[1])

    cursor.execute("SELECT file_id FROM episodes WHERE id=?", (eid,))
...
async def buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif query.startswith("series_"):
        return await show_episodes(update, context)

    elif query.startswith("ep_"):
        return await send_episode(update, context)
...
def main():
    ...
    app.add_handler(MessageHandler(filters.TEXT & filters.ALL, send_broadcast))
