...
BOT_TOKEN = os.getenv("BOT_TOKEN")  # ❗ TOKEN endi Renderdan olinadi
ADMIN_ID = 7343837323
...
async def save_movie(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cursor.execute("CREATE TABLE IF NOT EXISTS movies(code TEXT, name TEXT, file_id TEXT)")
...
async def save_request(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cursor.execute("CREATE TABLE IF NOT EXISTS requests(user_id INTEGER, text TEXT)")
...
async def send_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.user_data.get("broadcast"):
        ...
        for u in users:   # ❗ BU JOY XATO EDI — tuzatildi
            try:
...
if __name__ == "__main__":   # ❗ BU HAM XATO EDI
    main()
